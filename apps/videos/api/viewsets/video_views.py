...
from apps.videos.api.serializers.video_serializers import VideoSerializerPublic
...
class VideoPublicViewSet(viewsets.ReadOnlyModelViewSet):
    serializer_class = VideoSerializerPublic
    serializer_no_public_class = VideoSerializer2
    ...
    @action(detail=False, methods=['get'])
    def listVideosByTemporada(self, request,id):
            """Metodo para obtener la lista completa de videos clasificados como series registrados en la base de datos
            ...
            Returns.
                Response: Respuesta con la data y el estado de la respuesta (200 OK)
            """
            print("se validaran los videos de la serie numero ",id)
            video_serializer = self.serializer_class(self.get_queryset().filter(temporada = id), many=True)
            data = {
                "total": self.get_queryset().count(),
                "videos": video_serializer.data,
            }
    ...
    def playback_url(self, request, pk=None):
        """
        Retorna la URL de reproducción del video (restringido a usuarios autenticados)
        """
        video = self.get_queryset(pk)
        if video:
            return Response(
                {"url_vimeo": video.url_vimeo_esp},
